                ...
                <div ref="chatContainer" class="react-scroll-to-bottom--css-krija-1n7m0yu">
                          ...
                          <div class="w-[30px] flex flex-col relative items-end">
                            <div
                              ...
                              style="background-color: rgb(16, 163, 127);"><svg width="41" height="41" viewBox="0 0 41 41"
                                fill="none" xmlns="http://www.w3.org/2000/svg" stroke-width="1.5" class="h-6 w-6">
                                <path
                            ...
                            <div v-if="conv.speeches.length > 1"
                              ...
                              <button @click.stop="last(conv)" :disabled="!(conv.idx > 0)"
                                class="dark:text-white disabled:text-gray-300 dark:disabled:text-gray-400"><svg
                  ...
                  <transition name="el-fade-in-linear">
                    ...
                    <button v-show="isShowGoBottom" @click="handleScrollBottom"
                      class="cursor-pointer absolute right-6 bottom-[124px] md:bottom-[120px] z-10 rounded-full border border-gray-200 bg-gray-50 text-gray-600 dark:border-white/10 dark:bg-white/10 dark:text-gray-200"><svg
                  ...
                  <div class="flex flex-col gap-2 text-gray-100 text-sm">
                    ...
                    <template v-for="conversation, cidx in conversations">
                      ...
                      <a v-else-if="conversation.delete" @blur="cancelDelConv(cidx, conversation)"
                        class="m-focus flex py-3 px-3 items-center gap-3 relative rounded-md cursor-pointer break-all pr-14 bg-gray-800 hover:bg-gray-800 group"><svg
                          stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round"
                        ...
                        <div class="absolute flex right-1 z-10 text-gray-300 visible">
                          ...
                          <button @click="cancelDelConv(cidx, conversation)" class="p-1 hover:text-white"><svg
                              stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24"
                              stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4" height="1em" width="1em"
                              xmlns="http://www.w3.org/2000/svg">
                              <line x1="18" y1="6" x2="6" y2="18"></line>
                      ...
                      <a v-else @click.stop.prevent="selectConversation(conversation)"
                        ...
                        <div v-show="conversation.selected" class="absolute flex right-1 z-10 text-gray-300 visible">
                          <button @click="editTitle(cidx, conversation)" class="p-1 hover:text-white"><svg
                              stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24"
                              stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4" height="1em" width="1em"
                              xmlns="http://www.w3.org/2000/svg">
                          ...
                          <button @click="conversation.delete = true" class="p-1 hover:text-white"><svg
                              stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24"
                              stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4" height="1em" width="1em"
                              xmlns="http://www.w3.org/2000/svg">
                              <polyline points="3 6 5 6 21 6"></polyline>
                  ...
                  <div class="flex items-center sm:flex">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:text-left">
                      <h3 class="text-lg font-medium leading-6 text-gray-900 dark:text-gray-200"
                        id="headlessui-dialog-title-:r5:" data-headlessui-state="open">Provide additional feedback</h3>
                    </div>
                  </div>
...
export default {
  ...
  methods: {
    ...
    chatRepeat() {
      ...
      conv["idx"] = conv["suitable"].length;
      conv["loading"] = true;
      conv["suitable"].push(0);
      conv["speeches"].push("");
      that.refrechConversation()

      this.axios.post(`/api/chat/repeat/${this.cid}`, {})
        .then((result) => {
          console.log(result);
    ...
    selectConversation(conv) {
      ...
      this.axios.get(`/api/conv/${conv.id}`)
        .then((result) => {
          ...
          setTimeout(() => { that.isScrollAndNotBottom(); }, 300)
        })
        .catch((err) => { });
    },
    editTitle(idx, conv) {
      ...
      setTimeout(() => { document.getElementById("titleInput").focus(); }, 150)
    },
    titleInputBlur(idx, conv) {
      setTimeout(() => { this.cancelChangeConvTitle(idx, conv); }, 100);
    },
